...
if [ "$TARGET_OS" = "ubuntu" ]; then
    ...
    # metapackage that pulls whichever PHP the base release ships, and both
    # it and libapache2-mod-fcgid enable themselves via their dh_apache2
    # postinst -- phpX.Y.load and fcgid.load are already symlinked into
    # mods-enabled before any a2enmod could run. So there's no
    ...
    # Naming no PHP version above is deliberate: the last one named here went
    # stale at a base bump, which is the failure this branch exists to avoid.
    apt-get -yq install \
    ...
    # the Ubuntu base ships neither wget nor curl -- so without this every
